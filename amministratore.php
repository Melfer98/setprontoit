...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true) {
    header("Location: login_amministratore.php");
    exit;
}
...
<body>
  <header>
    <?php require_once 'header_amministratore.php'?>
  </header>
  <main>
    <br>
      ...
      <?php foreach ($anuncios as $anuncio) : ?>
        <div class="announcement">
          ...
          <a href="profilo.php?user=<?= base64_encode($anuncio['idUtente']) ?>&<?= $anuncio['nome'] ?>&<?= $anuncio['cognome'] ?>">
            ...
            <button onclick="window.location.href='visualizza_annuncio_in_approvazione.php?id=<?= $anuncio['idAnnuncio']; ?>'" class="button b-visualizza">Visualizza tutto</button>
          </div>
          <div class="right-column col-md-6">
            ...
            <div class="d-flex justify-content-end mt-4">
                <button class="btn-filter b-approvati m-2" onclick="ApprovaAnnuncio(<?php echo $anuncio['idAnnuncio']; ?>)">Approvato</button>
                <button class="btn-filter b-nonapprovati m-2" onclick="NonApprovaAnnuncio(<?php echo $anuncio['idAnnuncio']; ?>)">Non Approvato</button>
            </div>
          </div>
        </div>
      <?php endforeach; ?>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content" style="width:650px">
            <div class="modal-header">
                <h5 class="modal-title" id="commentModalLabel">Inserisci un commento</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <textarea id="commentoInput" class="form-control" rows="16" placeholder="Inserisci il commento"></textarea>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Annulla</button>
                <button type="button" id="salvaCommentoBtn" class="btn btn-primary">Salva commento</button>
            </div>
        </div>
    </div>
</div>
...
function NonApprovaAnnuncio(idAnnuncio) {
    // Apri il popup per l'inserimento del commento
    $('#commentModal').modal('show');

    // Aggiungi un gestore di eventi al pulsante "Salva commento"
    $('#salvaCommentoBtn').on('click', function() {
        const commento = $('#commentoInput').val();

        // Esegui una richiesta AJAX per salvare il commento nella base di dati
        const xhr = new XMLHttpRequest();
            ...
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    if (response.success) {
                        ...
                        // Visualizza un messaggio di errore
                        alert("Errore durante il salvataggio del commento: " + response.message);
                    }
                } else {
                    // Visualizza un messaggio di errore in caso di errore nella richiesta AJAX
                    alert("Errore durante la richiesta AJAX per il salvataggio del commento.");
                }
            }
        };
        xhr.send("idAnnuncio=" + idAnnuncio + "&commento=" + commento);
    });
}
